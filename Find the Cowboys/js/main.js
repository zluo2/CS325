...
function preload() {
  ...
  game.load.audio('music','assets/fighting.mp3');
  game.load.audio('score1','assets/score.mp3');
}
...
function create() {
  ...
  //添加分数计量器->X,Y,分数默认值,字体大小，字体填充颜色
	scoreText = game.add.text(16,16,'Star score:0',{ fontSize : '32px',fill: '#FFF'});
	diamText = game.add.text(500,16,'Diamond score:0',{ fontSize : '32px',fill: 'red'});
}
...
function collectStar (player,star,diamond){
  ...
  score1.play();
  score += 10;
  scoreText.text = 'Star score:' +score ;
}
...
function collectDiam (player,diamond) {
  ...
  score1.play();
  diam += 20;
  diamText.text = 'Diamond score:' +diam;
}
